...
import { CreateLessonPlanDto, FindLessonPlanDto, UpdateLessonPlanDto, SectionInputDto } from "./dtos";
import { Transaction } from "@/core/decorators/transaction.decorator";
import {
  WordRepo, LessonPlanRepo, GameRepo, TaskRepo,
  LessonPlanVocabRepo, LessonPlanGrammarRepo,
  LessonPlanListeningRepo, LessonPlanWritingRepo,
} from "@/repositories";
import { LessonPlanVocab, LessonPlanGrammar, LessonPlanListening, LessonPlanWriting } from "@/entities";
...
export class LessonPlanService {
    ...
    private async loadSections(lpIds: string[]) {
        const [vocabs, grammars, listenings, writings] = await Promise.all([
    ...
    private async resolveRef(refs: { refId?: string; refType?: NSLessonPlan.ELessonPlanType }[], withWords = false) {
        ...
        const [games, tasks] = await Promise.all([
            gameIds.length ? this.gameRepo.find({
                where: { id: In(gameIds) },
                ...(withWords ? { relations: { words: true }, select: { id: true, type: true, words: wordSelect } } : {}),
            }) : [],
            taskIds.length ? this.taskRepo.find({
                where: { id: In(taskIds) },
                ...(withWords ? { relations: { words: true }, select: { id: true, name: true, words: wordSelect } } : {}),
            }) : [],
        ]);
        ...
        return (ref?: { refId?: string; refType?: NSLessonPlan.ELessonPlanType }) => {
            ...
            return ref.refType === NSLessonPlan.ELessonPlanType.GAME
                ? gameMap.get(ref.refId) ?? null
                : taskMap.get(ref.refId) ?? null;
        };
    }
    ...
    private flattenSections(lpId: string, sectionMap: Map<string, any>, resolve: (ref?: any) => any) {
        ...
        if (!s) {
            return { vocab: null, vocabType: null, grammar: null, grammarType: null, listening: null, listeningType: null, writing: null, writingType: null };
        }
        return {
    ...
    private async createSection(
        lessonPlanId: string,
        section?: SectionInputDto,
    ): Promise<{ refId: string; refType: NSLessonPlan.ELessonPlanType } | null> {
    ...
    async update(id: string, dto: UpdateLessonPlanDto) {
        ...
        const upsertSection = async (
            repo: LessonPlanVocabRepo | LessonPlanGrammarRepo | LessonPlanListeningRepo | LessonPlanWritingRepo,
